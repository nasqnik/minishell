...
/*   Updated: 2025/01/07 14:35:50 by meid             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    char *stro = ft_substr(str, 1, ft_strlen(str) - 2);
    return (stro);
}
...
t_tree *parsing_subshell(t_info *info)
{
    ...
    char *new_buffer = ft_strtrim_sides(info->buffer);
    ...
    t_tree *subshell = create_ast_tree(&tokens);
    // print_list(info->token_list);
	if (info->ast_tree == NULL)
		return(handle_error(info, "creat_tree", 2, '\0'), NULL); // meybe we dont need that
