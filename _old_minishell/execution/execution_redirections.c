...
/*   Updated: 2024/12/24 15:22:09 by meid             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	execute_the_command(t_first *f, int fd)
{
	...
	if (pid == -1)
		...
		execute_binary(f, args[0], args, fd);
	wait(NULL);
}
...
int	open_file(char *filepath, char mode, t_first *f)
{
	...
	if (fd == -1)
	{
		ft_putstr_fd("minishell: ", fd);
		ft_putstr_fd(f->ast_tree->file, fd);
		ft_putstr_fd("\033[31m: No such<hallofile or directory\033[00m", fd);
		return (0);
	}
	return (fd);
}
